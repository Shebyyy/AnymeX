...
class _ListExporterPageState extends State<ListExporterPage> {
  ...
  Future<String?> _getMalCsrfToken() async {
    try {
      // First, get the MAL export page to extract CSRF token
  ...
  Future<void> _exportAnilistList() async {
    ...
    // GraphQL query to fetch user's list
    final query = '''
    ...
    if (response.statusCode == 200) {
      ...
      // Convert to XML format (similar to MAL export format for compatibility)
